...
- [ ] **7.7 — Raiz Inteligente (§6a) — FASE 2** 🔴 ABERTA — **decisões TRAVADAS (Vinicius):** ramificação **binária** · **STUDENT-collab = staff** · lista de workspaces inline **só mediante SENHA validada + rate-limit desde o dia um** · **reuso** de producer-login + student-workspaces · **assume o caso aluno-sem-workspace** (hoje coberto pela mensagem honesta do 7.6). Base mapeada nas investigações #1-#3 do item (proxy role-blind e presence-blind; producer-login já pratica confirma-só-pós-auth; student-workspaces tem o agrupamento anti-enum pronto).

> **FAMÍLIA "hardcode ignora personalização" (report noturno 2026-07-16, reportado por Ana Luiza / ws `shop-club`, prints):** 3 itens (7.9/7.10/7.11) da MESMA doença — o produtor customiza uma cor e o texto/elemento ignora, hardcoded. Censo unificado por namespace na próxima sessão (1 comando read-only). Ordem de ataque de TODA a fila noturna: **7.9 (B) → 7.10+7.11 (D+E, censo unificado) → 7.12 (A) → 7.13 (C)** — as migrações de A e C podem agrupar. Investigação-mãe do 7.9/7.12/7.13 feita em 2026-07-16 (file:line abaixo).

- [ ] **7.9 — [B] Cor de texto da VITRINE não pega** 🔴 BUG de promessa quebrada — o campo `vitrineTextColor` EXISTE e é editável (aba Aparência, rótulo "Texto — Cor do texto principal", `appearance-tab.tsx:118`), MAS é aplicado num lugar só e ERRADO: `w/[slug]/layout.tsx:47` alimenta **apenas `--producer-button-text`** (texto de botão). NÃO há var de texto de corpo (`--vitrine-text`/`--producer-text`) na injeção SSR. **Censo dos ~10 textos hardcoded** (todos `text-gray-*`/`text-white`, ignoram o campo) em `w/[slug]/page.tsx`: "Meus cursos" (:484,:512, o reportado) · "Outros cursos" (:561) · título boas-vindas h2 (:346,:359) · subtítulo (:349,:362) · título de categoria h3 (:455) · empty "Nenhum curso encontrado" (:437) · empty "não está matriculado" (:519,:523) · input de busca+placeholder (:398) · chips de categoria (:407-420). **ZERO schema** (o campo já existe — é wire-up: injetar `--vitrine-text` + trocar os hardcodes). ⚠️ **Risco: especificidade `.dark`** (skill L2/L8/L9 — `.dark` guard, Tailwind token não pega var sozinho). **PRIORIDADE 1 da família.**
- [ ] **7.10 — [D] Badge do sininho hardcoded (azul)** 🟡 — mesma família do 7.9; o badge de notificação ignora o tema (azul fixo). **Censo PENDENTE** (roteiro D1-D4, não investigado ainda): D1 localizar o componente do badge · D2 mapear as superfícies onde aparece (header producer/admin/vitrine) · D3 decidir a var por namespace (`--producer-*` vs `--member-*` vs `--vitrine-*`) · D4 contraste do número dentro do badge. Fazer no censo unificado com o 7.11.
- [ ] **7.11 — [E] Painel do PRODUTOR ignora o tema** 🔴 — superfície GRANDE (Dashboard, Meus Alunos, Relatórios, Suporte, editor de curso); mesma família; **censo PENDENTE** por namespace `--producer-*`. ⚠️ **Decisão de produto no desenho (mockup ANTES de código):** cores **SEMÂNTICAS** que devem PRESERVAR significado (verde=receita, vermelho=alerta/reembolso, badge "Publicado"=status) vs cores que devem SEGUIR o tema — separar as duas famílias antes de tocar. Não é "trocar tudo por var".
- [ ] **7.12 — [A] Fade do banner do CURSO** 🟡 — feature nova com molde **100% pronto** = o `vitrineBannerFade*` da vitrine (schema `vitrineBannerFadeEnabled/Color/Opacity` :103-105 · editor `edit/page.tsx:84-86,410` · **render exato a copiar** `w/[slug]/page.tsx:304-343`: hex-validado → `linear-gradient(to top, cor, rgba .3, transparent)` + opacity; senão o gradiente branco/gray-950 default). Hoje o fade do curso é **hardcoded** em `(course)/course/[slug]/page.tsx:444-459` (2 overlays fixos branco/`gray-950`, sem controle). **Exige 3 colunas novas no Course** (`courseBannerFade*`, o Course não tem — só tem `bannerUrl`/`bannerPosition` + 6 `member*`) + **migração (Runbook completo)** + PUT `/api/courses/[id]` + aba do editor de curso + o render. Default `true` = zero mudança visual em curso existente.
- [ ] **7.13 — [C] Cor de texto do LOGIN (override)** 🟢 — ⚠️ HOJE **não é bug**: o login **auto-deriva** a cor do texto da luminância da box (`getLoginTheme`, `workspace-auth-shell.tsx:113-116` → `--wa-text*` :286-289; box clara→texto escuro, escura→branco). **Decisão do Vinicius: override explícito** = 1 campo novo `loginTextColor` + wire no `--wa-text`; o **auto-derive atual vira FALLBACK** quando o campo estiver vazio (não some). Exige migração (agrupável com o 7.12). O único texto do login que já tem campo próprio é o link "Esqueci minha senha" (`loginLinkColor`).
- [ ] **7.16 — 🔴 Histórico de Webhooks VAZA entre workspaces do mesmo produtor** — ISOLAMENTO DE DADOS (gravidade acima da família de tema). Um workspace recém-criado (zero integração, card "Inativo", zero tokens/mapeamento) exibe os **50 eventos do workspace IRMÃO** — com **emails de compradores e externalIds** na tela errada. Report: Ana Luiza / `shop-club`, print 2026-07-16; o sintoma documentado é a contradição **"Inativo + 25 ok/25 erro na mesma tela"**. **HIPÓTESE a provar (não assumir):** a query do histórico filtra por **PRODUTOR (ownerId)**, não por **workspaceId**. **Investigação read-only PRIMEIRO** deve responder: **(a)** o filtro errado vaza SÓ entre irmãos do mesmo dono, ou existe cenário PIOR (cross-tenant entre produtores diferentes)? **(b)** a MESMA query serve outra superfície (o `integrations/webhook-logs` do 1.4 já teve vazamento cross-tenant de PII — `7d6c8b8`; verificar se é a mesma rota ou prima)? Família cross-tenant (lição dos **moderators do lives** `443a4dd` + do **webhook-logs do 1.4** — validar o PAYLOAD, não só o status). ⚠️ Se (a) = cross-tenant entre donos, **sobe pra prioridade máxima** (LGPD, PII de pagamento). Provar file:line + a query real antes de qualquer desenho.
- [ ] **7.14 — Erros de upload genéricos (thumbnail/banner do curso · capa do módulo)** 🟡 — "Erro ao fazer upload" sem o motivo real (tamanho/formato/rede). Report: produtor `shop-club`, prints 2026-07-15. **Investigação read-only PRIMEIRO** (como sempre — L1 dado antes de código, L4 reusar o que já existe). Dois ângulos no desenho: **(1) UX** — propagar o motivo real ao usuário (o componente engole o erro, padrão do BUG A: `if(res.ok)` sem else / `catch {}` vazio — verificar se é o mesmo). **(2) ⚠️ BUG por trás — HIPÓTESE A PROVAR:** os hints prometem 5MB mas a **Vercel corta o body em ~4.5MB** → a janela **4.5–5MB falha "dentro do limite"** (413 `FUNCTION_PAYLOAD_TOO_LARGE` mascarado de erro genérico). **Mesmo território do BUG A** (`f2ea405`, que resolveu materiais >4.5MB com **upload direto browser→Supabase via signed URL**) — checar se thumbnail/banner/capa passam pela lambda (têm o teto) ou já vão diretos. Se passam pela lambda: o fix reusa o molde do signed-url do BUG A. ⚠️ NÃO assumir a hipótese — provar file:line + testar a janela 4.5–5MB antes de desenhar.
- [ ] **7.15 — Perfil do aluno (`/w/[slug]/profile`) ignora o tema do produtor** 🟡 — card "Alterar senha" + irmãos (botão, avatar, badge de nível, link "Alterar foto") em cores fixas — **família hardcode-vs-tema** (entra no censo unificado dos 7.9-7.11). ⚠️ **HIPÓTESE ARQUITETURAL a provar (não assumir):** a página é um wrapper de 17 linhas que **reusa o `ProfilePage` GLOBAL** (`w/[slug]/profile/page.tsx:4-15` → `import ProfilePage from "@/app/(dashboard)/profile/page"` com prop `workspaceSlug` — fato mapeado na investigação da Trava) — componente **compartilhado** entre o contexto global e o do workspace, possivelmente cego às vars `--vitrine-*`/`--member-*` de onde renderiza. Se confirmar: o fix é o componente consumir o tema do CONTEXTO em que renderiza (padrão NOVO da família — os 7.9-7.11 são hardcode simples; este é hardcode-por-compartilhamento). ⚠️ Cuidado extra: o mesmo componente serve o `/profile` global do staff (a TRAVA Opção 2 do BUG C vive nele — `isPureStudent return null`) — qualquer mudança valida os DOIS contextos. Report: Ronei / `shop-club`, print 2026-07-16.
