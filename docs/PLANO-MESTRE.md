...
> **✅ FILA NOTURNA 2026-07-16 — 8/8 EM PRODUÇÃO:** 7.9+7.15 (`462a24d`) · 7.10 (`6f27ee6`) · 7.11 (`875429e`) · 7.12+7.13 (`6a9f635`) · 7.16 (`944e152`) · 7.14 Fase A (`874d251`). Todo o report noturno da Ana Luiza/`shop-club` fechado e deployado; abertos só os follow-ups estruturais (7.14 Fase B signed-url) e o épico 7.7 Raiz Inteligente.
>
...
- [ ] **7.14 FASE B — Upload de imagem direto (signed-url) 🟡 ESTRUTURAL** — a Fase A baixou o limite honesto pra 4MB, mas o teto ~4.5MB da lambda **continua** (banners grandes ainda barram). Reusar o **molde signed-url do BUG A** (`materials/signed-url` + `createSignedUploadUrl`, `f2ea405`) nas 3 imagens: browser sobe **direto pro Supabase** → o teto some. Raio (mapa do P1 do 7.14): as 3 convergem no `/api/upload` (lambda) via `thumbnail-upload`/`banner-upload`; o molde já existe em `materials/signed-url`. **Destrava banners grandes de verdade.** Read-only primeiro (o fluxo do signed-url do material como referência).
