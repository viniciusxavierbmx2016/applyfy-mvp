...
- [ ] **9.27 — ÉPICO: redesign da comunidade (desenhado ago/26)** 🟡 — 5 fases aprovadas em protótipo, nível Apple/Nubank conforme decidido. **F1 card de post**: badge de tipo só quando ≠ FREE (**79% dos posts são FREE** — 22 de 28 medidos, e a pílula "Livre" não informa nada), "Fixado" vira estado do card, grupo vira texto, 3 ícones viram `···`, mídia full-bleed, paleta reduzida ao acento do produtor (**29 das 50 classes de cor do card não seguem o tema** — a maioria é amber + a paleta de 4 cores do tipo, e só blue/emerald são remapeados). **F2 composer**: repouso em 1 linha (hoje nasce com 88px vazios), barra 14→6 botões, ⭐ foto como miniatura 64px FORA do fluxo de texto (hoje entra como nó no meio do conteúdo, empurra o escrito e move o cursor), botão explica por que está apagado. **F3 comentários**: aninhamento por espaçamento (hoje `ml-8` + `border-l-2` come 1/3 da largura em 375px), balão derivado do card (hoje `dark:bg-gray-800/60`, uma das classes descobertas), 2 níveis máx, selo "Respondendo a X", rótulo de moderação (hoje o pendente aparece sem explicação e o `commentCount` o ignora — `posts/route.ts:124` filtra `APPROVED`). ⚠️ SEM curtir em comentário — `Comment` não tem relação `Like` no schema. **F4 barra de grupos**: resolve o 9.20 — `overflow-hidden` + `translateX`, trava de eixo aos 8px, `touchmove` NÃO passivo, véu nas bordas, setas+wheel no desktop (a barra hoje não tem mecânica de desktop nenhuma), nome truncado em 210px, aba ativa trazida para a viewport. **F5 feed**: 1 passagem esqueleto→conteúdo (hoje são 5 e o post pisca e some), vazios por permissão, fim de lista marcado, erro visível (hoje o `catch(() => {})` engole e prende o aluno no feed misto). ⚠️ Motion é CSS (`fadeIn`/`scaleIn`/`fadeInUp` + `slide-up`) — a skill da casa vence as skills externas de animação em produção. ⚠️ O `PostCard` recebe 8 props e nenhum índice: animação escalonada exige passá-lo. **F1a `ccf8fcb` · F1b `181cc2f` · F2 `76a3a41` · F4 `7cfbd49` (+`243a714`) · F3b `de5bba2` — FEITAS.** Resta a **F5** (feed). ⚠️ A F3 do plano previa "2 níveis" e "rótulo de moderação" que **já existiam** (backend reancora em `comments/route.ts:145`, selo no `CommentBlock`); a fase mexeu só na aparência, e os defeitos funcionais que a investigação achou viraram **9.31-9.34**. ⚠️ Para a F5: **fade no wrapper, nunca stagger por card** — stagger cria contexto de empilhamento em cada card e prende o menu `···`, fazendo o card seguinte pintar por cima dele.

- [ ] **9.31 — Comentários falham em silêncio** 🔴 — `submitComment` (`post-card.tsx:267`), `submitReply` (`:288`) e `deleteComment` (`:315`) só agem `if (res.ok)`. Falha de rede ou 500: o botão volta ao normal, o texto some e nada explica. `deleteComment` não tem nem `try/finally`. É perda de trabalho do usuário sem aviso — o defeito mais grave que a investigação da F3 achou, e mais sério que o recuo visual que a fase corrigiu.

- [ ] **9.32 — "Responder" aparece em comentário pendente e dá 403 mudo** 🟠 — `comments/route.ts:143` recusa resposta a comentário `PENDING`, mas o front mostra o botão em qualquer comentário raiz, inclusive num pendente do próprio autor. O clique não produz nada visível. Hoje é teórico (17 comentários, **todos APPROVED** — medido ago/26), arma quando a moderação entrar em uso.
