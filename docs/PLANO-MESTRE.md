...
- [ ] **9.35 — Comunidade sem margem VERTICAL (e o projeto sem safe-area)** 🟠 — o fix de `243a714` deu à página `px-4 sm:px-6 lg:px-8`, **só horizontal**. Sem `py`/`pb`, o último post encosta no fim da tela. A home e a aula usam `p-4 lg:p-8`, que já cobre o bottom — a comunidade é a única com padding só no eixo X. ⚠️ **Pior no PWA iOS**: a faixa do home indicator come o rodapé e o conteúdo fica embaixo dela. ⚠️ **Verificado (ago/26): o projeto NÃO trata safe-area em lugar nenhum** — zero ocorrências de `safe-area-inset`/`env(safe`/`viewport-fit` em todo o `src/`, e o meta servido em produção é `width=device-width, initial-scale=1`, **sem `viewport-fit=cover`**. Consequência: acrescentar `env(safe-area-inset-bottom)` sozinho **não faz nada** — sem `viewport-fit=cover` o `env()` resolve para 0. São duas mudanças e a primeira é do shell/layout raiz, não da comunidade: (1) `viewport-fit=cover` no viewport, (2) o `pb` com `env()` nas telas. A (1) muda o enquadramento de TODA a área do aluno em iPhone com notch — fatiar e validar antes.
