...
⚠️ **Status honesto dos 3 REVOKE_EVENTS (mesma situação da Kiwify):** `refund`, `chargeback` e `subscription_canceled` são **nome ASSUMIDO** — comentado no adapter. Só o GRANT (`purchase_approved`) está provado por payload real. **Se algum divergir, o REVOKE cai em IGNORE silenciosamente e o aluno reembolsado MANTÉM o acesso** — isso é receita, não cosmético. **Candidato: confirmar por captura real.**
