...
> **Abertos:** 2.4 PARCIAL (rate limit — **Peça A shared-store Upstash ✅ `75c07be`**; abertas: **Peça B origin lockdown** + **constant-time login** do G2 da Raiz; stopgap **2.4a** ✅ `c3bad5a`) · 2.5 (CSP `unsafe-inline`/`unsafe-eval`).
...
### 2.4 — Rate limiting: store compartilhado + origem 🔴 PARCIAL (2.4a stopgap ✅ `c3bad5a` · **Peça A shared-store ✅ `75c07be`** · Peça B lockdown + constant-time ABERTAS)
...
- [ ] **Peça B — origin lockdown** (header-guard Cloudflare→origin, ou corte do `*.vercel.app`) — ⚠️ tratar os 2 vetores de quebra da investigação (webhooks README `*.vercel.app` + Vercel Cron direto) + checklist de painel (Cloudflare Transform Rule / Vercel domínios).
- [ ] **constant-time login** (do G2 da Raiz) — mascarar o oráculo de existência do GoTrue (pré-existente).
**Dependência:** nenhuma; a Peça B é config-de-painel-pesada (o desenho já mapeou o furo do proxy no-op pra `/api/` e o CRON_SECRET pra isentar o cron).
