...
>
> **Estado:** `main` em `c5f2f7a` (+ docs-commit desta atualização) · auditoria de segurança crítica FECHADA · plataforma em produção com clientes pagantes.
> **Última atualização:** 2026-07-18 (FASE 6.0 Fatia 2 core `c5f2f7a` — lib comum + adapter Hubla + rota; verify() real; falta a UI/Fatia 3).
...
- [ ] ⏳ **FATIA 3 — UI + CONFIG** (`<HublaCard>` no hub + a tela `/producer/settings/integrations/hubla` + o endpoint de secret cifrado via `encrypt()` + o mapeamento produto→curso `gateway="hubla"`) — o que dá ao produtor a URL `/api/webhooks/hubla/{slug}` e o campo pra cadastrar o token (hoje semeado à mão no staging). **É o que resta pra Hubla ponta-a-ponta.**
**Dependência:** 5.2 (visibilidade da tela). **Estado: Fatias 1 (schema) + 2 (core) FEITAS e em prod (rota Hubla viva mas inerte); falta a Fatia 3 (UI) pra o produtor configurar.**
