...
- **Upload honesto (7.14 Fase A)** ✅ `874d251` — hint mentia (5MB vs teto ~4.5MB da lambda) + 2 fronts com `res.json()` antes do `res.ok` (413 mascarado); fix = helper `upload-image.ts` (guarda client 4MB + leitura defensiva), 5MB→4MB nas 2 pontas. **Fila noturna 2026-07-16 = 8/8 em prod.** Fase B (signed-url × 3 imagens) aberta. Ver PM 7.14.
