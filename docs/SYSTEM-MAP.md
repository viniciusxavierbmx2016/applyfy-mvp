...
**ABERTO:**
- 1.5, 1.6 (convite) · 2.4 PARCIAL (Peça A shared-store ✅ `75c07be` com chave por-rota · **Peça B.1 origin lockdown OBSERVE ✅ `53748b2` FINALIZADA 2026-07-18** — vigia em Node grava no-stamp/webhook-external/exempt-cron nas 10 rotas, SEMPRE passa, tela `/admin/origin-lock`; env LIGADA: `ORIGIN_LOCK_SECRET` na Vercel + Transform Rule 'Origin Lock Stamp' no Cloudflare, carimbo provado em prod (nav do admin silenciosa; único no-stamp foi o login pré-propagação); **abertas: Peça B.2 enforce** — agora só depende da janela de observação limpa (sem webhook-external), fail-OPEN sem env — **+ constant-time login** do G2 da Raiz) · 2.5 (CSP) · 4.5 (console.error) · FASE 3 (email A retry + B EmailLog) · FASE 5 quick-wins (5.1 custom domain / 5.2 admin-nav integrations) · **FASE 6 (épico multi-gateway) — 6.0 fundação: 8 decisões travadas + contrato `GatewayAdapter`/`processGatewayWebhook` desenhado (Hubla = 1º gateway). ⭐ FATIA 1 (SCHEMA) ✅ `50edb79` (CourseExternalProduct +gateway, WorkspaceGatewaySecret cifrada; Applyfy byte-idêntico provado, 30 mapeamentos backfillados) + ⭐ FATIA 2 (CORE) ✅ `c5f2f7a` (lib comum `processGatewayWebhook` extraída da escopada + adapter Hubla + rota `/api/webhooks/hubla/[slug]`; verify() REAL: x-hubla-token vs secret cifrado, x-hubla-idempotency no dedup, leitura defensiva user??payer; recordTransaction OFF sem amount; matriz a-f verde por payload; Applyfy byte-idêntico na rota dele). ⚠️ **rota Hubla VIVA mas INERTE em prod** — sem WorkspaceGatewaySecret(hubla) configurado (a UI é a Fatia 3), POST sem secret = recusa limpa {received:true} 200 (não 500, provado). ⚠️ risco cross-gateway ACEITO (Applyfy resolve sem filtro gateway [slug]:66; blindar tocaria o Applyfy, decisão 7 veta). **Falta a FATIA 3 (UI+config)** pro produtor cadastrar o token. 6.2 multi-token ✅ já implementado; 6.3 = greenfield (só entrada)** · FASE 9 (débito/QA).
