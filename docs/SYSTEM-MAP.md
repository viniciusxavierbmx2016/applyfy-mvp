...
- BUG A materials upload `f2ea405` · **BUG B** admin-collab dashboard `4ad99f5` · **BUG C** troca senha aluno `9fac2d9` · **1.2/1.3** rateLimit 2 rotas `c3bad5a` · **5.4** CSV import no editor de curso `2c2ef5b` · **5.3** toggle box de info do curso `943b8e4` (client demand #3).
- **BUG C-irmão** 🟢 housekeeping (hipótese "tranca" refutada; achados: código morto + comentário errado + resend sem senha).

**ABERTO:**
- 1.5, 1.6 (convite) · 2.4 (rate-limit; **input**: balde compartilhado por segmento `w` — precisa chave por-rota) · 2.5 (CSP) · 4.5 (console.error) · FASE 3 (email A retry + B EmailLog) · FASE 5 quick-wins (5.1 custom domain / 5.2 admin-nav integrations) · FASE 6 (épico multi-gateway) · FASE 9 (débito/QA).
