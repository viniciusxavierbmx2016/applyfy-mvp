...
- **Precedência sobre skills externas de animação.** Se as skills de `.claude/skills/` (emil-design-eng, apple-design, animate, review-animations, improve-animations) recomendarem Motion/Framer Motion, esta skill vence em código de produção: o padrão da casa é CSS — keyframes existentes (`globals.css:64-75`, `tailwind.config.ts:57-63`) + transitions do Tailwind. Instalar motion library é decisão de produto explícita, nunca efeito colateral. A recomendação delas vale no Modo B (protótipos), não no Modo A (produção) — é essa a distinção entre a regra acima e a menção a Motion mais adiante.
