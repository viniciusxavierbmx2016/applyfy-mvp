...
description: Design and implement member-facing frontend for the Members Club platform (app.mymembersclub.com.br) — a Brazilian membership-area SaaS for infoprodutos (course dashboards, lesson players, progress/trilha, community, profile, paywall, onboarding). Use this skill whenever the user asks to build, design, restyle, or review any screen of the member experience — even if they say "área de membros", "área do aluno", "player de aula", "tela de curso", "trilha", "dashboard do membro", or name specific competitors (Hotmart Club, MemberKit, Kajabi, Skool). Two modes — production code honors the codebase's real systems (--member-*/--producer-* token namespaces, .dark class theming, producer-set white-label colors, CSS-only motion, inline SVG icons, PWA/iOS constraints); prototypes and artifacts use the full aspirational system. Enforces a bold/disruptive Brazilian aesthetic in the Nubank/Canva/iFood lineage with billion-dollar polish, dark-mode-first, mobile-first, Portuguese-aware. Triggers across PT-BR and EN phrasings.
---
