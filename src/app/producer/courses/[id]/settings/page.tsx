      ...
      {loading || !flags ? (
        <div className="space-y-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="h-24 rounded-xl bg-gray-100 dark:bg-gray-900 animate-pulse"
            />
          ))}
        </div>
      ) : (
        <div className="space-y-3">
          {ITEMS.map((item) => {
            ...
            return (
              <article
                key={item.key}
                className={`flex items-start gap-4 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/50 p-5 transition ${
                  enabled ? "" : "opacity-70"
                }`}
              >
                <span
